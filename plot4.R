plot4 <- function() {
	...
	pc <- pc[pc$Date=="2/1/2007" | pc$Date=="2/2/2007",]
	png("plot4.png",width=480,height=480)
	par(mfrow=c(2,2))
	# Upper left
	plot(strptime(paste(pc[,1],pc[,2]),format="%m/%d/%Y %H:%M:%S"),
	...
	lines(strptime(paste(pc[,1],pc[,2]),format="%m/%d/%Y %H:%M:%S"),pc$Sub_metering_2,type="l",
		ylim=c(0,30),col="blue")
	lines(strptime(paste(pc[,1],pc[,2]),format="%m/%d/%Y %H:%M:%S"),pc$Sub_metering_3,type="l",
		ylim=c(0,30),col="red")
